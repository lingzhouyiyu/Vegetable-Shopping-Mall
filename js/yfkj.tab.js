...
appcan.define("tab", function(a, b, c) {
	var d = '<%if(!option.ispico){%><div class="ulev-1 ub ub-f1" data-index="<%=index%>">    <div class="ub-f1 ub ub-ver item ub-ac sc-text-tab">    <%if(option.hasIcon){%><div class="ub-f1 fa <%=data.icon%> fa-2x tx-c"></div><%}%>    <%if(option.hasLabel){%><div class="ulev tx-c"><%=data.label%></div><%}%>    </div>    <%if(option.hasBadge && data.badge){%><div class="ulev bc-text-head sc-bg-alert tab_badge uhide" id="<%="badge_"+index%>"><%=data.badge%></div><%}%>    </div><%}else{%>'+
	'<div class="ulev-1 ub ub-f1" data-index="<%=index%>">'+
	'<div class="ub-f1 ub ub-ver item ub-ac sc-text-tab">'+
	'<div class="ub-img pico<%=index%>" style="height: 2em;width: 2em; background-image: url(<%=data.pico%>);"></div>'+
	'<div class="ulev tx-c"><%=data.label%></div>'+
	' <%if(option.hasBadge && data.badge){%><div class="ulev bc-text-head sc-bg-alert tab_badge uhide" id="<%="badge_"+index%>"></div><%}%>'+
	'</div></div></div>'+
	'<%}%>',
	...
	f.prototype = {
		...
		focusText: function(b) {		    
			...
			if (!(c.option.hasAnim && c.option.hasIcon && c.option.hasLabel)) {
				var d = a(".item", c.ele);
				if(c.option.ispico){
				  $(".pico"+tabyfkjtag).css("background-image","url("+c.option.data[tabyfkjtag].pico+")")
				  $(".pico"+b).css("background-image","url("+c.option.data[b].pactivico+")")
                  tabyfkjtag=b;
				}
